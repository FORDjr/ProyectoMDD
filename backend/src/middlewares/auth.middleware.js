/**
 * Middleware para verificar si el usuario es administrador o encargado
...
async function isManager(req, res, next) {
  ...
  try{
    ...
    //Obtiene el rol del usuario en la sesión
    const userRole = req.session.user.rolName;
    ...
    console.log("Error en auth.middleware.js -> isManager(): ", error);
    return res.status(500).json({ message: 'Error interno del servidor' });
  }
}
